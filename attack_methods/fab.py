...
class FAB(Attack):
    ...
    """
    def __init__(self, model, args, norm='Linf', eps=None,
                 alpha_max=0.1, eta=1.05, beta=0.9, verbose=False):
        super().__init__("FAB", model)
        self.norm = norm
        self.n_restarts = args.fab_n_restarts
        Default_EPS_DICT_BY_NORM = {'Linf': .3, 'L2': 1., 'L1': 5.0}
        self.eps = eps if eps is not None else Default_EPS_DICT_BY_NORM[norm]
        self.alpha_max = alpha_max
        self.eta = eta
        self.beta = beta
        self.steps = args.fab_step
        self.targeted = False
        self.verbose = verbose
        self.seed = args.seed
        self.target_class = None
        self.n_target_classes = args.fab_num_class - 1
    ...
    def forward(self, imgs, labels):
        ...
        adv_images = self.perturb(imgs, labels)
